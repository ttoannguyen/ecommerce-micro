...
`db/migration/V1__init.sql` là nguồn sự thật. Hibernate chạy `validate` — đối chiếu, không viết DDL.
...
1. ~~Flyway~~ — xong
2. Reservation thành entity + TTL (hold hết hạn tự trả hàng)
3. Release theo `reservationId` + `Idempotency-Key` (mở đường retry an toàn)
4. WireMock test cho Feign
5. Kafka + outbox
6. Eureka + Gateway + Resilience4j + Zipkin
